...
public class CheckLoginInterceptor extends HandlerInterceptorAdapter {
	 ...
	 * preHandle() : 해당 컨트롤러(Handler) 가 실행되기 전 수행되는 메서드 
	       리턴하는 정보가 true 이면 해당 컨트롤러(Handler) 실행되고,
	       리턴하는 boolean 정보가 false 이면 해당 컨트롤러(Handler)가 실행되지 않는다. 
	   
	       현 인터셉터가 반영될 대상 지정은  spring-web.xml 에서 설정한다. 
	       
	       로그인 인증 상태가 아닌 사용자는 
	       로그인 폼이 있는 home.jsp로 이동시킨다. (redirect)
	       또한 해당 Handler (Controller)는 실행하여 서비스할 필요가 없으므로 false를 리턴한다.
	 */
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(session==null||session.getAttribute("mvo")==null) {
			...
			System.out.println("**HandlerInterceptor preHandler 실행**" +uri);
