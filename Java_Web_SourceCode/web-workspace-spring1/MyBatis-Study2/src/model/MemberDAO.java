...
public class MemberDAO {
	...
	public void updateMember(MemberVO vo) {
		...
		try {
			session = factory.openSession();
			int a = session.update("member.updateMember",vo);
			session.commit();
			System.out.println("몇명 업데이트 됬나 ? : "+a);
		}finally {
			session.close();
		}
	}
	public int deleteMemberByAddressAndName(HashMap<String, String> map) {
		SqlSession session = null;
		int a=0;
		try {
			session= factory.openSession();
			a= session.delete("member.deleteMemberByAddressAndName",map);
			session.commit();
		}finally {
			session.close();
		}
