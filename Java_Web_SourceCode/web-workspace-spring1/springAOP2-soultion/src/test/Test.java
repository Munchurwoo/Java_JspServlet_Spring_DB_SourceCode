 ...
 *        ---> 대안 :  AOP 다~~~~	
 *        AOP 적용단계 
 *        1. 횡단(or 공통) 관심사항을 정의한 클래스를 구현 
 *        aop.common.LoggingAspect
 *        logging(JoinPoint)
 *        
 *        2. spring-config.xml 에 aop 설정 
 *        	1) 횡단관심사항을 정의한 클래스를 bean 으로 생성 
 *        	2) pointcut : 적용대상을 지정  -AspectJ 표현식 excution 으로 지정 
 *        	3) advice : 메서드 실행전이므로 before advice를 설정  
 */
public class Test {
